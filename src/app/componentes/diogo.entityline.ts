...
import { TitleCase, FlashMessageService, EntityComponent, AngularmService } from 'angularm';


@Component({
    selector: 'tr [mgEntityLine]',
    template:
        `<td [ngClass]="'w3-center'" *ngFor="let property of entity.mountProperties()">{{property.value}}</td>
         <td> <a href="#" (click)="show()">Show</a></td>
         <td> <a href="#" (click)="edit()">Edit</a></td>
         <td> <a href="#" (click)="destroy()">Destroy</a></td>`,
})
export class DiogoEntityLineComponent extends EntityComponent {
